...
export class Gpio extends ConfigItem {
    ...
    public async feedDeviceValue(binding: string | DeviceBinding, data: any) {
        try {
            let bind = typeof binding === 'string' ? new DeviceBinding(binding) : binding;
            // Find the pinId.
            if (isNaN(bind.deviceId)) return Promise.reject(new Error(`feedDeviceValue: Invalid pin #${bind.binding}`));
            let pin = this.pins.find(elem => elem.id === bind.deviceId);
            if (typeof pin === 'undefined') return Promise.reject(new Error(`feedDeviceValue: Pin #${bind.deviceId} not found.`));
    ...
    public async getDevice(binding: string | DeviceBinding) {
        try {
            let bind = typeof binding === 'string' ? new DeviceBinding(binding) : binding;
            // Find the pinId.
            if (isNaN(bind.deviceId)) return Promise.reject(new Error(`getDevice: Invalid pin #${bind.binding}`));
            let pin = this.pins.find(elem => elem.id === bind.deviceId);
            if (typeof pin === 'undefined') return Promise.reject(new Error(`getDevice: Pin #${bind.deviceId} not found.`));
            return pin;
        }
        catch (err) { return Promise.reject(new Error(`Could not set gpio state: ${err}`)); }
    }
    public async getDeviceStatus(binding: string | DeviceBinding) {
        try {
            let bind = typeof binding === 'string' ? new DeviceBinding(binding) : binding;
            // Find the pinId.
            if (isNaN(bind.deviceId)) return Promise.reject(new Error(`getDeviceStatus: Invalid pin #${bind.binding}`));
            let pin = this.pins.find(elem => elem.id === bind.deviceId);
            if (typeof pin === 'undefined') return Promise.reject(new Error(`getDeviceStatus: Pin #${bind.deviceId} not found.`));
    ...
    public getDeviceInputs(): any[] {
        ...
        for (let i = 0; i < this.channels.length; i++) {
            ...
            if (typeof dev !== 'undefined' && typeof dev.inputs !== 'undefined') {
                devices.push({ uid: `spi:${this.busNumber}:${channel.id}`, id: channel.id, name: ad.name, deviceId: ad.id, type: 'spi', busNumber: this.busNumber, bindings: dev.inputs });
            }
        }
        return devices;
    }
...
export class GpioPin extends ConfigItem {
    ...
    public async getDeviceStatus(): Promise<any> {
        try {
            ...
            if (typeof p === 'undefined') return Promise.reject(new Error(`getDeviceStatus: Could not find active Pin #${this.headerId}-${this.id}`));
    ...
    public async setDeviceState(binding: string | DeviceBinding, data: any): Promise<any> {
        try {
            ...
            if (isNaN(latch)) return Promise.reject(new Error(`setDeviceState: GPIO Pin #${this.headerId} - ${this.id} latch data is invalid ${data.latch}.`));
            let p = gpioPins.pins.find(elem => elem.headerId === this.headerId && elem.pinId === this.id);
    ...
    public async feedDeviceValue(binding: string | DeviceBinding, data: any): Promise<any> {
        try {
            ...
            // At this point we have the current value.
            let latch = (typeof data.latch !== 'undefined') ? parseInt(data.latch, 10) : -1;
            if (isNaN(latch)) return Promise.reject(`feedDeviceValue: GPIO Pin #${this.headerId} - ${this.id} latch data is invalid ${data.latch}.`);
            let p = gpioPins.pins.find(elem => elem.headerId === this.headerId && elem.pinId === this.id);
            let oldState = await p.readPinAsync();
            // Now that the state has been read lets set its state.
            let newState = typeof data.state !== 'undefined' ? utils.makeBool(data.state) : typeof data.isOn !== 'undefined' ? utils.makeBool(data.isOn) : false;
...
export class GenericDeviceController extends ConfigItem {
    ...
    public async deleteBus(bus): Promise<I2cBus> {
        try {
            /*             let b = (typeof bus.Id !== 'undefined') ? this.devices.getItemById(bus.id) : typeof bus.busNumber !== 'undefined' ? this.devices.getItemByBusNumber(bus.busNumber) : undefined;
                        if (typeof b === 'undefined') return Promise.reject(`Could not find bus by bus #${bus.busNumber} or id ${bus.busId}`);
                        await b.closeAsync();
                        this.devices.removeItemById(b.id);
                        return Promise.resolve(bus); */
        }
        catch (err) { return Promise.reject(err); }
    }
    public getDeviceInputs(): any[] {
        let devices = [];
        /*         for (let i = 0; i < this.devices.length; i++) {
                    let bus = this.devices.getItemByIndex(i);
                    devices.push(...bus.getDeviceInputs());
                } */
        return devices;
    }
...
export class GenericDevice extends ConfigItem {
    ...
    public get address(): number { return this.data.address; }
    public set address(val: number) { this.setDataVal('address', val); }
    public get feeds(): GenericDeviceFeedCollection { return new GenericDeviceFeedCollection(this.data, 'feeds'); }
    public get options(): any { return typeof this.data.options === 'undefined' ? this.data.options = {} : this.data.options; }
    public set options(val: any) { this.setDataVal('options', val || {}); }
    public get info(): any { return typeof this.data.info === 'undefined' ? this.data.info = {} : this.data.info; }
    public set info(val: any) { this.setDataVal('info', val || {}); }
    ...
    public async setDeviceFeed(data): Promise<GenericDeviceFeed> {
        try {
            let feedId = typeof data.id !== 'undefined' ? parseInt(data.id, 10) : typeof data.feedId !== 'undefined' ? parseInt(data.feedId, 10) : -1;
            if (isNaN(feedId)) return Promise.reject(`The feed identifier is not valid.`);
            let feed: GenericDeviceFeed;
            let connectionId;
            let connection;
            if (feedId !== -1) {
                // We are updating.
    ...
    public async deleteDeviceFeed(data): Promise<GenericDeviceFeed> {
        try {
            let feedId = typeof data.id !== 'undefined' ? parseInt(data.id, 10) : typeof data.feedId !== 'undefined' ? parseInt(data.feedId, 10) : -1;
            if (isNaN(feedId)) return Promise.reject(`The feed identifier is not valid.`);
            let feed: GenericDeviceFeed;
...
export class GenericDeviceFeedCollection extends ConfigItemCollection<GenericDeviceFeed> {
    constructor(data: any, name?: string) { super(data, name || 'feeds') }
    public createItem(data: any): GenericDeviceFeed { return new GenericDeviceFeed(data); }
}
export class GenericDeviceFeed extends ConfigItem {
    constructor(data) { super(data); }
    public initData(data?: any) {
        if (typeof this.data.isActive === 'undefined') this.isActive = false;
        if (typeof this.data.changesOnly === 'undefined') this.changesOnly = true;
